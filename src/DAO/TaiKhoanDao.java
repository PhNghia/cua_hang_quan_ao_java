...
public class TaiKhoanDao {
    ...
    public ArrayList<NhanVien> getEmployees() {
        ...
        String query = "SELECT nhan_vien.* FROM nhan_vien LEFT JOIN tai_khoan ON " +
        "nhan_vien.ma_nhan_vien = tai_khoan.ma_nhan_vien WHERE tai_khoan.ma_nhan_vien IS NULL AND " + 
        "nhan_vien.trang_thai = 1 AND nhan_vien.hien_thi = 1";
        try {
            ResultSet result = this.connection.executeQuery(query);
            if (result != null) {
                while (result.next()) {
